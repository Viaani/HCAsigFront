...
import { eliminarConvalidacionAction } from '../../Redux/Acciones/ConvalidacionAction';
//reacr-pdf
import { Page, Text, View, Document, StyleSheet, Image, Font, pdf } from '@react-pdf/renderer';
import styled from '@react-pdf/styled-components';
//Componentes
// import Boton from '../General/Boton';
// import MostrarEquivalencia from '../Comparar/MostrarEquivalencia';

const VerConvalidacion = ({convalidacion, indexVerConvalidacion}) => {
//---------------Atributos---------------
    ...
    //---------------Funciones---------------
    const eliminarConvalidacion = (id) => {
        clienteAxios.delete(`/api/Convalidacion/${id}`,{
            headers: {
                'Authorization': token
              }
        })
            .then(respuesta => {
                dispatch(eliminarConvalidacionAction(id))
                dispatch(alertaAction({mensaje:"Exito al eliminar Homologación", segundos:2, color:"verde"}))
            ...
            .catch(error=>{
                if(error.response !== undefined){
                    if(error.response.data === "TokenNoValido"){
                        ...
                        dispatch(alertaAction({mensaje:"Error al eliminar Homologación", segundos:4, color:"rojo" }))
                    }
                }else{
    ...
    const EnviarConvalidacion = (destinatarioSeleccionado) =>{
                        ...
                        <View style={styles.linea}>
                            ...
                            <View style={styles.col8}>
                                <Text style={styles.textCuadro}>
                                    {convalidacion.nombre+" "+convalidacion.apellido}
                                </Text>
                                <Text style={styles.textCuadro}>
                                    {convalidacion.run}
                                </Text>
                                <Text style={styles.textCuadro}>
                                    {convalidacion.programaOrigen.nombre}
                                </Text>
                                <Text style={styles.textCuadro}>
                                    {convalidacion.programaObjetivo.nombre}
                                </Text>
                            </View>
                        </View>
                    </View>
        ...
        .then(respuesta=>{
            ...
            clienteAxios.post(`/api/Solicitud/OficiarConvalidacion/${destinatarioSeleccionado}/${usuario.email}`, formData,{
            headers: {
                'Authorization': token
              }
            })
    ...
    const HandleEnviarConvalidacion = e => {
        e.preventDefault();
        setLoading(true)
        EnviarConvalidacion(destinatarioSeleccionado)
    }
//---------------Retorno---------------
    return (
        <Card>
            <Card.Header as ={Row}>
                <Col>
                    <Accordion.Toggle as = {Table} eventKey={indexVerConvalidacion} striped bordered responsive hover>
                        <thead>
                            <tr>
                                <th>Rut</th>
                                <th>Nombre</th>
                        ...
                        <tbody>
                            <tr>
                                <td>{convalidacion.run}</td>
                                <td>{convalidacion.nombre+" "+convalidacion.apellido}</td>
                                <td>{convalidacion.programaOrigen.nombre}</td>
                                <td>{convalidacion.programaObjetivo.nombre}</td>
                            </tr>
                        </tbody>
                    </Accordion.Toggle>
                </Col>
                <Col xs = {2}>
                    <ButtonGroup as ={Form} onSubmit = {HandleEnviarConvalidacion} vertical>
                        <ButtonGroup vertical>
                            ...
                            <Button
                                title = "Enviar Homologación a Secretario Administrativo"
                                type = "submit"
                                className="colorBH"
                                disabled={isLoading}
                            >
                        ...
                        <Button title="Eliminar Homologación" onClick={()=>{eliminarConvalidacion(convalidacion.run)}} variant="danger"><i className="fas fa-trash-alt"></i></Button>
                    </ButtonGroup>
                </Col>
            </Card.Header>
            <Accordion.Collapse eventKey={indexVerConvalidacion}>
                <Card.Body>
                    <Table striped bordered responsive hover>
                        ...
                        <tbody>
                            {
                                convalidacion.listaAsignaturasEquivalentes !== null &&
                                    convalidacion.listaAsignaturasEquivalentes.map((equivalencia, index)=>(
                                        <tr key = {index}>
                                            <td>{index+1}</td>
                                            <td>
...
export default VerConvalidacion;