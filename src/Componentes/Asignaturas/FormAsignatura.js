...
import { AgregarAsignaturaAction, EditarAsignaturaAction } from '../../Redux/Acciones/DecretosAction';
...
const FormAsignatura = ({indexFormAsignatura, editar, asignaturaProp}) => {
    //---------------Atributos---------------
    const dispatch = useDispatch();
    const token =  useSelector((state)=>state.usuariosStore.dataUsuarioConectado.token)
    const [validated, setValidated] = useState(false);
    const [asignatura, setAsignatura] = useState({
        codigo:asignaturaProp.codigo,
        nombre:asignaturaProp.nombre,
        creditos:asignaturaProp.creditos,
        numeroDecreto:asignaturaProp.numeroDecreto
    });
    var editandoAsignatura = asignaturaProp.codigo
    //---------------Funciones---------------
    const agregarAsignatura = (asignatura) => {
        var formData = new FormData()
        formData.append("archivo", document.getElementById("inputArchivo_"+indexFormAsignatura).files[0])
        clienteAxios.post(`/api/Asignatura/${asignatura.codigo},${asignatura.nombre},${asignatura.creditos},${asignatura.numeroDecreto}`, formData,{
            headers: {
                'Authorization': token
              }
        })
        .then(respuesta => {
            ...
            dispatch(AgregarAsignaturaAction(asignatura))
            setValidated(false)
            // ---------------Cerrar Modal---------------
            document.getElementById("inputArchivo_"+indexFormAsignatura).value = "";
            setAsignatura({
                codigo:'',
                nombre:'',
                creditos:'',
    ...
    const editarAsignaturaArchivo = (id, asignatura) => {
        ...
        .then(respuesta =>{
            ...
            dispatch(EditarAsignaturaAction(id, asignatura))
            setValidated(false)
            // ---------------Cerrar Modal---------------
    ...
    const editarAsignatura = (id, asignatura) => {
        console.log(asignatura);
        clienteAxios.put(`/api/Asignatura/${id}`, asignatura,{
            headers: {
                'Authorization': token
              }
        })
    ...
    const submitNuevoAsignatura = e =>{
        ...
        if (form.checkValidity() === true){
            if((document.getElementById("inputArchivo_"+indexFormAsignatura).files[0].type) !== "application/pdf")
            {
    ...
    const submitAsignaturaEditado = e =>{
        ...
        if (form.checkValidity() === true){
            if (document.getElementById("inputArchivo_"+indexFormAsignatura).files[0] === undefined){
        ...
        <React.Fragment>
            <Container> 
                <Row className="justify-content-md-center">
                    <Col md="8">
                        <h4> {editar?"LLene el formulario para editar una Asignatura":"LLene el formulario para agregar una Asignatura"}</h4>
                    </Col>
                </Row>
                <Row className="justify-content-md-center">
                    <Col md="8">
                        <br/>
                        <Form noValidate validated={validated} onSubmit={editar?submitAsignaturaEditado:submitNuevoAsignatura} encType="multipart/form-data" id ={"formArchivo_"+indexFormAsignatura} method="post">
                            ...
                            <Form.Group controlId="FormAsignaturaNombre">
                                ...
                                <Form.Control
                                    ...
                                    required
                                />
                                <Form.Control.Feedback type="invalid">Solo letras o paréntesis</Form.Control.Feedback>
                            </Form.Group>
                            {
                                editar?
                                    <Form.Group id="formcheck-api-regular">
                                        ...
                                        <Form.Control
                                            id = {"inputArchivo_"+indexFormAsignatura}
